...
(defun synset-uri (a)
   (resource (format nil "synset-~a" a) "wn30"))

(defun wordsense-uri (a b)
  (resource (format nil "wordsense-~a-~a" a b) "wn30"))

(defun get-synset-class (the-cls)
  (let ((trans `(("n" !wn20:NounSynset)
		 ("v" !wn20:VerbSynset)
		 ("a" !wn20:AdjectiveSynset)
		 ("s" !wn20:AdjectiveSatelliteSynset)
		 ("r" !wn20:AdverbSynset))))
    (cadr (assoc the-cls trans :test 'equal))))
